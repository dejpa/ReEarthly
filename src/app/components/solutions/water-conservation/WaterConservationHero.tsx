...
export default function WaterConservationHero() {
  const t = useTranslations()
  const pathname = usePathname()
  const locale = pathname.split('/')[1] || 'en'

  return (
    <section className="relative min-h-[80vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-cyan-900 via-blue-900 to-slate-800">
      ...
      <div className="absolute top-24 left-0 right-0 z-10">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          <div className="flex items-center text-sm text-slate-300">
            <Link href={`/${locale}`} className="hover:text-white transition-colors">{t('solutions.waterConservation.breadcrumb.home')}</Link>
            <span className="mx-2">/</span>
            <Link href={`/${locale}/solutions`} className="hover:text-white transition-colors">{t('solutions.waterConservation.breadcrumb.solutions')}</Link>
            <span className="mx-2">/</span>
            <span className="text-white font-medium">{t('solutions.waterConservation.breadcrumb.current')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-32 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
          <button className="group relative px-10 py-5 bg-white text-slate-900 font-semibold text-lg rounded-2xl shadow-2xl hover:shadow-white/25 transition-all duration-300 transform hover:scale-105 hover:-translate-y-1">
            {t('solutions.waterConservation.hero.primaryCTA')}
          </button>
          ...
          <button className="group px-10 py-5 bg-white/10 backdrop-blur-md text-white font-semibold text-lg rounded-2xl border-2 border-white/30 hover:bg-white/20 transition-all duration-300 transform hover:scale-105">
            {t('solutions.waterConservation.hero.secondaryCTA')}
          </button>
        </div>
      </div>
    </section>
